...
$$
{{< /katex >}}
(first term is to build the tree, the second is to query for the nearest point in the tree)
which $O(n^2 \log(n))$.
...
That's great, but there's no free lunch: on the second query of each "round", we now have to request the **2 nearest points** of the query point (just in case the first nearest point from that round is again the closest point).
...
And because all we have to do is build and query k-d trees, one need not implement any complicated data structures, just the thin wrapper that tracks the rounds, triggers rebuilds, and keeps track of which points have been found so far.
Very easy and kinda fast!

---

## Notes
- The novelty of this approach is not that it achieves optimal running time (which I believe is $n \log(n)$), but that it achieves an asymptotic speedup while using only using off-the-shelf data structures.
  No internal modifications to the k-d tree are needed, it is purely used as a black-box, and in an implementation the proposed algorithm wraps around it.
