...
We will show that we can do this faster than the naive quadratic running time without having to *implement* any complicated data structures (we'll only have to wrap them).

---
### Two bad algorithms

1. Put all points in a linked list.
   For every query perform a linear search on the list and delete the nearest point.
   The cost is $O(n^2)$.
...
$$
{{< /katex >}}
(first term to build the tree, the second is to query for the nearest point in the tree)
which is $O(n^2 \log(n))$ overall.
...
That's great, but there's no free lunch: on the second query of each "round", we now have to request the **2 nearest points** of the query point.
We must request 2 points to guard against the possibility that the closest point for the second query is the point we used for the first query, but have not removed from the tree yet.

But why stop at 2? What if we rebuilt the tree every $b$ points?
...
$$
{{< /katex >}}
The term $(n-ib) \log(n-ib)$ is the cost of building a k-d tree with $n-ib$ points; in later rounds (larger $i$) this cost shrinks.
...
The asymptotic cost of the periodic rebuild strategy becomes $O(n^{\frac{3}{2}}\log{n})$ with this choice for $b$.

And because all we have to do is build and query k-d trees, one need not implement any complicated data structures, just the thin wrapper that tracks the rounds, triggers rebuilds, and records which points have been found so far.
Very easy and kinda fast!

---

## Notes
- The novelty of this approach is not that it achieves optimal running time (which I believe is $n \log(n)$), but that it achieves an asymptotic speedup while using only off-the-shelf data structures.
  No internal modifications to the k-d tree are needed, it is purely used in a black-box, and the proposed algorithm wraps around that.
